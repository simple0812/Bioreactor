...
{
    public class PausingStatus : StatusBase
    {
        public PausingStatus(ControllerBase controller) : base(controller)
        {
        }
        ...
        {
            Controller.ProcessPausingResult(data, comEventArgs);
            LogFactory.Create()
                .Info(
                    $"device{Controller.Device.DeviceId} {OldStatus}status receive Pausing feedback convert to {comEventArgs.DeviceStatus}");
        }
